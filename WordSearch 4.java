...
public class WordSearch {
  ...
  public class Task {
    ...
    public boolean diagonalSearch(String str) {
      char[] s = str.toUpperCase().toCharArray();
      boolean flag = false;
      for (int r=0; r<grid.length - s.length; r++) {
        for (int c=0; c<grid[r].length - s.length; c++) {
          boolean f = true;
          for (int i=0; i<s.length; i++) {
            if (grid[r+i][c+i] != s[i]) f = false;
          }
          if (f == true) flag = true;
        }
      }
    ...
    public boolean diagonalSearchTranspose(String str) {
      char[] s = str.toUpperCase().toCharArray();
      boolean flag = false;
      for (int r=0; r<transposegrid.length - s.length; r++) {
        for (int c=0; c<transposegrid[r].length - s.length; c++) {
          boolean f = true;
          for (int i=0; i<s.length; i++) {
            if (transposegrid[r+i][c+i] != s[i]) f = false;
          }
          if (f == true) flag = true;
        }
      }
    ...
    public boolean searchGrid(String str) {
      ...
      if (flag == false) {
        flag = diagonalSearch(str);
      }
      if (flag == false) {
      }
        flag = diagonalSearchTranspose(str);
      return flag;
    }

...
public class Patriots extends JFrame {
  ...
  class Practice extends JPanel {
    ...
    class GamePanel extends JPanel {
      ...
      public void paintComponent(Graphics g) {
        super.paintComponent(g);
        int tm = (int)Duration.between(start_time, Instant.now()).getSeconds();
        //Image stickman = new ImageIcon("stickman.jpg").getImage();
        //Image startPageBackground = new ImageIcon("frontPageBackground.jpg").getImage();
        //g.drawImage(startPageBackground, 0, 0, 900,900, null);
        ...
        g.drawImage(coin, 790, 30, 50,50, null);
        setFont(new Font("Airacobra", Font.BOLD, 30));

        g.drawString(String.valueOf(tm),450,65);

        if (soldier_bullets > 0) {
          g.drawString(String.valueOf(soldier_bullets),850,65);
        } else {
          g.drawString("You win the game",550,65);
          Toolkit.getDefaultToolkit().beep();
        }


        if (robot_bullets > 0) {
          g.drawString(String.valueOf(robot_bullets),150,65);
        } else {
          g.drawString("You lost the game",150,65);
        }
